...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpCall:
			numArs := code.ReadUint8(ins[ip+1:])
			vm.currentFrame().ip += 1 // skip info about no. of arguments;

			err := vm.callFunction(int(numArs))
			if err != nil {
				return err
			}
...
func (vm *VM) buildHash(startIndex int, endIndex int) (object.Object, error) {
	hashedPairs := make(map[object.HashKey]object.HahsPair, endIndex-startIndex)

	for i := startIndex; i < endIndex; i += 2 {
		key := vm.stack[i]
		value := vm.stack[i+1]

		pair := object.HahsPair{Key: key, Value: value}
...
func (vm *VM) executeIndexExpression(left object.Object, index object.Object) error {
	...
	default:
		return fmt.Errorf("index operator not supprted: %s", left.Type())
	}
}
